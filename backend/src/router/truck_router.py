...
from src.controller.truck_controller import TruckController, get_truck_controller
...
@truck_router.post("")
def create_truck(
    dto: CreateTruckDto, controller: TruckController = Depends(get_truck_controller)
):
    return controller.create_truck(dto)


@truck_router.put("/{truck_id}")
def update_truck_weight(
    truck_id: UUID,
    dto: UpdateTruckWeightDto,
    controller: TruckController = Depends(get_truck_controller),
):
